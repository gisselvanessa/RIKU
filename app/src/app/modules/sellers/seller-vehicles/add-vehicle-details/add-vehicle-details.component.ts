...
import { EditVehicleService } from '../../../../shared/services/edit-vehicle.service';
...
import { isValidCedulaId } from '../../../loan-procedure/loan-helper';

@Component({
  selector: 'app-add-seller-vehicle-details',
  templateUrl: './add-vehicle-details.component.html',
  styleUrls: ['./add-vehicle-details.component.scss'],
})

export class AddSellerVehicleDetailsComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.nextYear = nextYearDate.getFullYear();
  }
  ngAfterViewInit() {
    setTimeout(() => {
      if (this.vehicleId) {
        ...
        const basicVehicleDetails = localStorage.getItem('vehicleObj');
        if (basicVehicleDetails) {
          this.basicVehicleDetails = JSON.parse(basicVehicleDetails);
        }
      }
    }, 1000);
  }
  ...
  public getVehicle(vehicle_id: string) {
    if (vehicle_id) {
      this.loading = true;
      this.editVehicleService.getVehicle(vehicle_id).subscribe(
        (res: any) => {
          this.loading = false;
  ...
  createForms() {
    try {
      ...
      this.basicDetailsForm = this.fb.group({
        ...
        distance_travelled_unit: ['Kilometers', Validators.required],
  ...
  setFormControlValues() {
    ...
    this.setProvince(this.vehicleDetail.location.province);
  ...
  getProvience(): Array<any> {
    this.addVehicleService.getProvinceList().subscribe(
      (res: any) => {
  ...
  next() {
    ...
    if (this.step == 1) {
      ...
      this.submitting = true;
      if (this.basicDetailsForm.invalid || !this.isValidCedula) {
        return;
      }
      this.submitting = false;
      this.step++;
      ...
      this.step++;
      this.orderDetail = {
        ...this.basicVehicleDetails,
        ...this.basicDetailsForm.value,
        photos: this.allPhotoUrls 
      };
    }
  }
  ...
  onSubmit(draft: boolean) {
    ...
    // this.specificationsForm.value.mileage = parseFloat((this.specificationsForm.value.mileage).replace(",", ''))
    ...
    if (!draft) {
      ...
      // this.specificationsForm.get('mileage_unit')?.setValue(this.mileageUnit);
      if (this.basicDetailsForm.invalid) {
    ...
    const addUpdateVehicle = this.vehicleId
      ? this.addVehicleService.updateSellerVehicle(filterdata)
      : this.addVehicleService.postSellerAddVehicle(filterdata);
  ...
  async mergePhotoUrls() {
    this.allPhotoUrls = [this.primaryPhotoUrl, ...this.otherPhotoUrls.map(photo => photo.src)];
    return this.allPhotoUrls;
}
