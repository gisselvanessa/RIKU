...
@Component({
  selector: 'app-add-seller-vehicle',
  templateUrl: './add-vehicle.component.html',
  styleUrls: ['./add-vehicle.component.scss']
})

export class AddSellerVehicleComponent implements OnInit, OnChanges {

  makeId: any;
  typeId: any;
  selectedDuration: any;
  sendDuration: any;
  selectDuration: any[] = [{ value: '1', option: '1 month' }, { value: '2', option: '2 month' }, { value: '3', option: '3 month' }, { value: '4', option: '4 month' }, { value: '5', option: '5 month' }]
  modelId: any;
  vehicleTypes: any[] = [];
  vehicleMakes: any[] = [];
  popularMakes: any[] = [];
  ...
  ngOnInit(): void {
    ...
    this.vehicleId = this.activatedRoute.snapshot.paramMap.get('id') || undefined;
  ...
  async preFillDetails(vehicleDetail: any) {
    ...
    const index = this.selectDuration.findIndex((x: any) => x.value == vehicleDetail.duration)
  ...
  public getVehicleTypes() {
    this.addVehicleService.getVehicleTypes().subscribe((res: any) => {
  ...
  public getVehicleMakeYear(make_id: any) {
    ...
    this.addVehicleService.getMakeYears(make_id).subscribe((res: any) => {
      ...
      this.addVehicleService.changeVehicle({
        'make_id': make_id ? make_id : this.vehicleDetail ? this.vehicleDetail.make_id : '',
        'vehicle_type': this.selectedVehicleType,
        'type_id': this.typeId,
        'vehicle_make': this.selectedVehicleMake,
        'make_year': this.selectedVehicleMakeYear ? this.selectedVehicleMakeYear : 0,
        'model_id': this.vehicleDetail ? this.vehicleDetail.model_id : '',
        'vehicle_model': this.selectedVehicleModel,
        'duration': this.sendDuration
      });
  ...
  public getVehicleModel(year: any) {
    ...
    this.addVehicleService.getModels(this.makeId, year).subscribe((res: any) => {
      ...
      this.addVehicleService.changeVehicle({
        'make_id': this.makeId ? this.makeId : this.vehicleDetail ? this.vehicleDetail.make_id : '',
        'vehicle_type': this.selectedVehicleType,
        'type_id': this.typeId,
        'vehicle_make': this.selectedVehicleMake,
        'make_year': year,
  ...
  public setVehicleModel(modelId: string) {
    ...
    this.addVehicleService.changeVehicle({
      ...
      'vehicle_make': this.selectedVehicleMake,
      'make_year': this.selectedVehicleMakeYear,
      'model_id': modelId != '' ? modelId : this.vehicleDetail ? this.vehicleDetail.model_id : '',
      'vehicle_model': this.selectedVehicleModel,
      'duration': this.sendDuration
    });
  }
  ...
  public submitAddForm() {
    if ((this.selectedVehicleType == undefined || this.selectedVehicleType == '')) {
    ...
    } else if (!this.selectedVehicleMakeYear || this.selectedVehicleMakeYear == '') {
   ...
   /*  } else if (!this.selectedDuration || this.selectedDuration == '') {
      this.toastr.error(this.translate.instant('Please select Duration of Months you want to keep posted')); */
    } else {
      let vehicleObj = {
        'make_id': this.makeId,
        'vehicle_type': this.selectedVehicleType,
      ...
      this.router.navigate(['/user/my-vehicles/add-vehicle-details']);
    }
    // if ((this.selectedVehicleType == undefined || this.selectedVehicleType == '') ||
    //   (this.selectedVehicleMake == undefined || this.selectedVehicleMake == '') ||
    //   (this.selectedVehicleMakeYear == undefined || this.selectedVehicleMakeYear == '') ||
    //   (this.selectedVehicleModel == undefined || this.selectedVehicleModel == '')) {
    ...
    //   this.addVehicleService.changeVehicle(vehicleObj);
    //   localStorage.setItem('vehicleObj', JSON.stringify(vehicleObj));
    //   this.router.navigate(['/add-vehicle-second']);
    // }
  }
