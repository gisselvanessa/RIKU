...
export class EarningsComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    // Push JSON to array
    //  for (let key in this.data.chartData) {
    //   if (this.data.chartData.hasOwnProperty(key)) {
    //     this.chartDataArray.push(this.data.chartData[key]);
    //   }
    //  }

     // Push JSON to options array
    // for (let key in this.jsonOptions.menuOptions) {
    //   if (this.jsonOptions.menuOptions.hasOwnProperty(key)) {
    //     this.optionsArray.push(this.jsonOptions.menuOptions[key]);
    //   }
    // }
  }
  ...
  createChart(chartData: any = []){
    let data: any,
    options: any,
    ctx: any = document.getElementById('dealerLineChart') as HTMLElement;
    let labels: any = [];
    let earnings: any = [];
    ...
    const customTooltips = (context: any) => {
      ...
      let tooltipEl: any = document.getElementById('dealer-earnings');
      if (tooltipEl) tooltipEl.remove();
      if (!tooltipEl) {
        tooltipEl = document.createElement('div');
        tooltipEl.id = 'dealer-earnings';
